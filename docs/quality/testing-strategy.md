...
3. Current coverage: interaction state transitions, key/switch link behavior, occupancy/blocking invariants, baseline line-of-sight/occlusion checks, directional hearing invariants, fog-of-war visible/explored state transitions, perception debug overlay payload generation, party selection/command-intent issuance invariants, abstract ability request validation/queueing invariants, abstract inventory item-use validation/queueing invariants, and baseline HUD metric/surface generation invariants.
